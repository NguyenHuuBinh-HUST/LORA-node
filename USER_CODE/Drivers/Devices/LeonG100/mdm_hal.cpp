...
//! check for timeout
#define TIMEOUT(t, ms)  ((ms != TIMEOUT_BLOCKING) && ((HAL_GetTick() - t) > ms))
...
inline void ARM_GPRS_TIMEOUT(uint32_t dur) {
    gprs_timeout_start = HAL_GetTick();
    gprs_timeout_duration = dur;
    DEBUG_D("GPRS WD Set %d\n",(dur));
}
inline bool IS_GPRS_TIMEOUT() {
    return gprs_timeout_duration && ((HAL_GetTick()-gprs_timeout_start)>gprs_timeout_duration);
}

inline void CLR_GPRS_TIMEOUT() {
    gprs_timeout_duration = 0;
    DEBUG_D("GPRS WD Cleared, was %d\n", gprs_timeout_duration);
}
...
#ifdef MDM_DEBUG
    if (_debugLevel >= 3) {
        DEBUG_D("%10.3f AT send    ", (HAL_GetTick()-_debugTime)*0.001);
        dumpAtCmd(buf,len);
    }
#endif
        ...
        if ((_debugLevel >= 3) && (ret != WAIT) && (ret != NOT_FOUND))
        {
            ...
            const char* s = (type == TYPE_UNKNOWN)? YEL "UNK" DEF :
                            ...
                            (type == TYPE_PROMPT) ? BLU " > " DEF :
                                                        "..." ;
            DEBUG_D("%10.3f AT read %s", (HAL_GetTick()-_debugTime)*0.001, s);
            dumpAtCmd(buf, len);
            (void)s;
        }
#endif
            ...
            if (type == TYPE_PLUS) {
                ...
                if (sscanf(cmd, "UUPSDD: %s", s) == 1) {
                    ...
                    if ( !strcmp(s, PROFILE) ) {
                        _ip = NOIP;
                        _attached = false;
                        DEBUG_D("PDP context deactivated remotely!\r\n");
...
void MDMParser::reset(void)
{
	...
	GPIO_Write(MDM_RESET, 0);
	HAL_Delay_ms(100);
	GPIO_Write(MDM_RESET, 1);
}
...
bool MDMParser::_powerOn(void)
{
    ...
    //Clear state pw_on
//    GPIO_Write(MDM_RESET, 0);
//    GPIO_Write(MDM_PWR, 0);
//    HAL_Delay_ms(1000);
//    GPIO_Write(MDM_RESET, 1);
//    GPIO_Write(MDM_PWR, 1);
    /* Initialize I/O */
    ...
    GPIO_Write(MDM_RESET, 1);
	  ...
	  HAL_Delay_ms(1000);
#if MDM_HARDWARE_FLOW_CONTROL_RTS_CTS
    _dev.lpm = LPM_ENABLED;
#else
    GPIO_Write(MDM_RTS, 0); // VERY IMPORTANT FOR CORRECT OPERATION W/O HW FLOW CONTROL!!
#endif
    GPIO_Write(MDM_DTR, 0);

    if (!_init) {
        MDM_INFO("[ SerialPipe::begin ] = = = = = = = =");

        /* Instantiate the USART hardware */
        thanhntMDM.start();
...
//        GPIO_Write(MDM_PWR, 0); HAL_Delay_ms(50);
//        GPIO_Write(MDM_PWR, 1); HAL_Delay_ms(10);

        // SARA-G35 >5ms, LISA-C2 > 150ms, LEON-G2 >5ms
        GPIO_Write(MDM_PWR, 0); HAL_Delay_ms(150);
        GPIO_Write(MDM_PWR, 1); HAL_Delay_ms(100);
...
bool MDMParser::powerOn(const char* simpin)
{
    ...
    for (int i = 0; (i < 5) && (_dev.sim != SIM_READY) && !_cancel_all_operations; i++) {
        ...
        if (_dev.sim == SIM_PIN) {
            ...
            uint32_t start = HAL_GetTick();
            while ((HAL_GetTick() - start < 1000UL) && !_cancel_all_operations); // just wait
        }
    }
...
int MDMParser::_cbATI(int type, const char* buf, int len, Dev* dev)
{
    if ((type == TYPE_UNKNOWN) && dev) {
        ...
        else if (strstr(buf, "LEON-G100")) *dev = DEV_LEON_G100;
    }
    return WAIT;
}
...
bool MDMParser::registerNet(NetStatus* status /*= NULL*/, uint32_t timeout_ms /*= 180000*/)
{
    LOCK();
    if (_init && _pwr) {
        MDM_INFO("\r\n[ Modem::register ] = = = = = = = = = = = = = =");
        ...
        if (checkNetStatus() == false) {
            ...
            while (!checkNetStatus(status) && !TIMEOUT(start, timeout_ms) && !_cancel_all_operations) {
                uint32_t start = HAL_GetTick();
                while ((HAL_GetTick() - start < 15000UL) && !_cancel_all_operations); // just wait
                //HAL_Delay_ms(15000);
            }
...
bool MDMParser::socketSetBlocking(int socket, uint32_t timeout_ms)
{
    bool ok = false;
    LOCK();
...
{
    MDM_INFO("\r\n[ Modem::devStatus ] = = = = = = = = = = = = = =");
    const char* txtDev[] = { "Unknown", "SARA-G350", "LISA-U200", "LISA-C200", "SARA-U260", "SARA-U270", "LEON-G200", "LEON-G100" };
...
MDM_DeviceSerial::MDM_DeviceSerial(int rxSize /*= 256*/) : SerialPipe(rxSize)
{
// Important to set _dev.lpm = LPM_ENABLED; when HW FLOW CONTROL enabled.
}

MDM_DeviceSerial::~MDM_DeviceSerial(void)
{
    powerOff();
    HW_UART2_DeInit();
}

void MDM_DeviceSerial::start(void)
{
	HW_UART2_Init();
}

int MDM_DeviceSerial::_send(const void* buf, int len)
{
    UART2_Send((const char*)buf, len);
    return 0;
}

int MDM_DeviceSerial::getLine(char* buffer, int length)
{
