...
//  uint8_t Rx_Buffer[BufferSize];
	uint8_t Tx_Buffer[35] = "Test communication with SPI FLASH!";//BufferSize = 34
	uint32_t BufferSize = CAL_SIZE(Tx_Buffer);
  uint8_t Rx_Buffer[CAL_SIZE(Tx_Buffer)];
  uint8_t Index = 0;
  uint32_t FlashID = 0;
  int TestStatus = -1;
